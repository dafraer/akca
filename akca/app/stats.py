...
TO_DATE=datetime.today().replace(hour=23, minute=59, second=59)
...
@click.pass_context
def stats(ctx, from_date: datetime, to_date: datetime, account: str, period: str):
    if period and (from_date or to_date):
        raise click.UsageError("Use either --period OR --from_date/--to_date, not both.")
    if period:
        p1 = f"this {period}"
        if period == "month":
            from_date = datetime.today().replace(day=1, hour=0, minute=0)
            to_date = TO_DATE
        else:
            from_date = datetime.today().replace(day=1, month=1, hour=0, minute=0)
            to_date = TO_DATE
