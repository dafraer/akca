...
@click.pass_context
def new(ctx, name: str, amount: float, date_: date, desc: str, category: str, account: str):
    try:
        id_ = ctx.obj.create_purchase(CreatePurchaseParams(name, amount, desc, category, date_.date(), account))
...
@click.pass_context
def edit(ctx, id: int, name: str, amount: float, description: str, category: str, date_: date, account: str):
    try:
        ctx.obj.edit_purchase(EditPurchaseParams(id, name, amount, description, category, date_.date() if date_ else None, account))
    except Exception as e:
